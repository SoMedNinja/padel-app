...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const job of jobs) {
      ...
      const participantDetails = participants.map(row => ({
        name: resolveName(row.profile_id),
        avatarUrl: row.profile_id ? profileMap.get(row.profile_id)?.avatar_url : null,
      }));
                        ...
                        <div class="email-card">
                          ...
                          ${podium.map(entry => `
                            <div class="email-avatar-row">
                              ...
                              <div>
                                <strong>${entry.rank}. ${entry.name || GUEST_LABEL}</strong>
