...
export default function Heatmap({ matches = [], profiles = [], allEloPlayers = [] }: HeatmapProps) {
  ...
  const frozenColumnWidth = useMemo(() => {
    ...
    // Note for non-coders: this keeps the frozen name column wide enough for the longest player name.
    return Math.min(360, Math.max(180, longestNameLength * 9 + 56));
  }, [sortedPlayerNames]);

  const validPlayers = useMemo(() => new Set(sortedPlayerNames), [sortedPlayerNames]);
  const eloMap = useMemo(
    () => new Map(allEloPlayers.map((player) => [normalizeProfileName(player.name), player.elo])),
    [allEloPlayers]
  );
  ...
  const pairStats = useMemo(() => {
    ...
    // Note for non-coders: each matrix cell represents how a pair of players performs when they are teammates.
        ...
        for (let i = 0; i < teamNames.length; i++) {
          for (let j = i + 1; j < teamNames.length; j++) {
            ...
            if (current) {
              ...
              const firstElo = eloMap.get(normalizeProfileName(first));
              const secondElo = eloMap.get(normalizeProfileName(second));
              const averageElo = typeof firstElo === "number" && typeof secondElo === "number"
                ? Math.round((firstElo + secondElo) / 2)
                : null;
    ...
    return stats;
  }, [resolvedMatches, validPlayers, eloMap]);
  ...
  const matrix = useMemo(() => {
    return sortedPlayerNames.map((rowName) =>
      sortedPlayerNames.map((colName) => {
        ...
        if (!stat) {
          return { matches: 0, winPct: null as number | null, elo: null as number | null, raw: null as number | null };
        }

        const winPct = stat.matches ? Math.round((stat.wins / stat.matches) * 100) : null;
        const raw = metric === "matches" ? stat.matches : metric === "winPct" ? winPct : stat.elo;
    ...
    <Card variant="outlined" sx={{ borderRadius: 3, boxShadow: "0 4px 12px rgba(0,0,0,0.04)", overflow: "hidden" }}>
      <CardContent sx={{ p: { xs: 2, sm: 3 } }}>
        ...
        <TableContainer component={Paper} variant="outlined" sx={{ borderRadius: 3, overflow: "auto", maxHeight: 520 }}>
          <Table size="small" aria-label="Heatmap med spelarkombinationer" sx={{ minWidth: frozenColumnWidth + sortedPlayerNames.length * 100 }}>
            <TableHead>
              <TableRow>
                <TableCell
                  sx={{
